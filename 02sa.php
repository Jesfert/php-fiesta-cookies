...
function sum($pokemonArray){
	$sum = 0;
	foreach($pokemonArray as $hpSum){
		$sum = $sum + $hpSum[5];
	}
	echo "The Sum of HP of all Pokemons is ",$sum,"<br>";
}
sum($Pabon);

function average($pokemonArray){
	$sum = 0;
	foreach($pokemonArray as $hpSum){
		$sum = $sum + $hpSum[6];
	}
	$ave = $sum / count($pokemonArray);
	echo "The Average attack of all Pokemons is ", $ave;
}
average($Pabon);
